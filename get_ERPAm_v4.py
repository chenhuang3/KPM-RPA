...
print "\nget other parameters from system1_kpm_1/rpa_kpm.ini ..."

# get number of freq 
cmd = "cat  system1_kpm_1/rpa_kpm.ini  | head -n 7 | tail -n 1 | awk '{print $1}'"
retval = os.popen( cmd ).read().rstrip('\r\n')
nfreq = int(retval)


cmd = "cat  system1_kpm_1/rpa_kpm.ini  | head -n 3 | tail -n 1 | awk '{print $1}'"
retval = os.popen( cmd ).read().rstrip('\r\n')
nMoment = int(retval)
...
cmdWeight = "grep freq_weight: "+dir1+str(1)+"/kpm.log.0"
...
for r in range(nRun):
    ...
    for i in range(nfreq):
        ...
        cmd1 = "grep \"random vector \" "+dir1+str(r+1)+"/kpm.log."+str(i)
        ...
        cmd2 = "grep \"random vector \" "+dir2+str(r+1)+"/kpm.log."+str(i)
...
print ">>> RPA energy vs moments (with ",nRV_moment," random vectors) <<<\n"
...
print "\n\n>>> Output files <<<"
print "\n ERPA_vs_nRV.txt   <= RPA energy versus number of random vectors"
